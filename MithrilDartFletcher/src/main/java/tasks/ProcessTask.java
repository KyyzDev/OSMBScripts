...
public class ProcessTask extends Task {
    ...
    // Uses the currently selected dart tip ID from MithrilDartFletcher.SelectedDartTipID
    private int[] sequenceFor(int tipId) {
        return new int[] { FeatherID, tipId, tipId, FeatherID };
    }
    ...
    public boolean activate() {
        ...
        ItemGroupResult inv = script.getWidgetManager().getInventory()
                .search(Set.of(FeatherID, tipId));
        return inv != null && inv.contains(FeatherID) && inv.contains(tipId);
    }
    ...
    public boolean execute() {
        ...
        ItemGroupResult inv = script.getWidgetManager().getInventory()
                .search(Set.of(FeatherID, tipId));
        if (inv == null || !inv.contains(FeatherID) || !inv.contains(tipId)) return false;
        ...
        final int[] SEQUENCE = sequenceFor(tipId);
        ...
        for (int n = 0; n < sequencesThisTick; n++) {
            for (int id : SEQUENCE) {
                ...
                final int firstId  = (lastClicked == FeatherID || id == FeatherID) ? FeatherID : tipId;
                final int secondId = (firstId == FeatherID) ? tipId : FeatherID;
    ...
    private boolean combinePair(ItemGroupResult inv, int firstId, int secondId, int tipId) {
        ...
        inv = script.getWidgetManager().getInventory().search(Set.of(FeatherID, tipId));
